...
> [!summary]
> Alexis King's essay introducing type-driven design through the slogan "parse, don't validate": push input checks to the boundary of your program and encode the results in precise types (e.g. `NonEmpty`, `Map`) so invariants are preserved in the type system and verified exactly once. It contrasts parsing (which preserves the information it learns) with validation (which throws it away, inviting "shotgun parsing"), and gives practical Haskell advice for making illegal states unrepresentable.
