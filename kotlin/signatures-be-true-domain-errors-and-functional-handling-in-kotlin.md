...
source: "https://blog.jetbrains.com/kotlin/2026/08/signatures-be-true-domain-errors-and-functional-handling-in-kotlin/"
author:
  - "[[Viliam Sedliak]]"
published: 2026-08-19
created: 2026-08-25
description: "Learn how to make expected failures visible in Kotlin function signatures with sealed error types and Either."
tags:
  - "clippings"
---

> [!summary]
> Makes the case that expected business failures belong in the function signature as `Either<DomainError, T>` rather than hidden in the body behind thrown exceptions. Splits failures into three groups: API client misuse (collapse into coarse 4xx responses), unexpected infrastructure exceptions (let them throw and become 500s), and true domain errors — the only group modeled as a sealed union in the contract. Domain errors stay typed through services and repositories and map to HTTP at exactly one place, the route boundary; the payoff is reviewing the contract instead of doing implementation archaeology, which matters more as agents draft the code underneath.

[Backend](https://blog.jetbrains.com/kotlin/category/backend/)
...
fun signDocument( documentId: UUID, code: String, ): Either<DocumentSignError, Unit>
...
fun signDocument(documentId: UUID, code: String) { if (signingWindowClosed(documentId)) throw SigningWindowClosedException() if (!codeMatches(documentId, code)) throw SignatureRejectedException() if (alreadySigned(documentId)) throw AlreadySignedException() //... sign it }
...
sealed interface DocumentSignError { data object SignatureRejected: DocumentSignError data object SigningWindowClosed: DocumentSignError data object AlreadySigned: DocumentSignError }
...
fun signDocument(documentId: UUID, code: String): Either<Throwable, Unit>
