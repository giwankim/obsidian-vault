...
source: "https://rxdb.info/articles/websockets-sse-polling-webrtc-webtransport.html"
author:
  - "[[RxDB]]"
...
> [!summary]
> Comparison of browser real-time transports written by the RxDB maintainer from building its sync engine: long-polling, WebSockets, Server-Sent Events, WebTransport (HTTP/3, still poorly supported) and WebRTC (needs a signaling server anyway). Covers practical limits such as the six-connections-per-domain cap under HTTP/1.1, mobile OSes killing background connections, corporate proxies blocking WebSockets, and clients missing events during reconnects. Recommends SSE for server-to-client push and WebSockets for bidirectional traffic, with long-polling only as a fallback.
...
This article aims to delve into these technologies, comparing their performance, highlighting their benefits and limitations, and offering recommendations for various use cases to help developers make informed decisions when building real-time web applications. It is a condensed summary of my gathered experience when I implemented the [RxDB Sync Engine](https://rxdb.info/replication.html) to be compatible with various backend technologies.
...
[WebRTC](https://webrtc.org/) (Web Real-Time Communication) is an open-source project and API standard that enables real-time communication (RTC) capabilities directly within web browsers and mobile applications without the need for complex server infrastructure or the installation of additional plugins. It supports peer-to-peer connections for streaming audio, video, and data exchange between browsers. [WebRTC](https://rxdb.info/replication-webrtc.html) is designed to work through NATs and firewalls, utilizing protocols like ICE, STUN, and TURN to establish a connection between peers.
...
The [RxDB Sync Engine](https://rxdb.info/replication.html) for example uses two modes of operation for that. One is the [checkpoint iteration mode](https://rxdb.info/replication.html#checkpoint-iteration) where normal http requests are used to iterate over backend data, until the client is in sync again. Then it can switch to [event observation mode](https://rxdb.info/replication.html#event-observation) where updates from the realtime-stream are used to keep the client in sync. Whenever a client disconnects or has any error, the replication shortly switches to [checkpoint iteration mode](https://rxdb.info/replication.html#checkpoint-iteration) until the client is in sync again. This method accounts for missed out events and ensures that clients can always sync to the exact equal state of the server.
...
What are the differences between long polling and traditional polling in web development?
...
Are server-sent events or WebSockets suitable for offline notifications?
...
- Learn how to use Server-Sent-Events to [replicate a client side RxDB database with your backend](https://rxdb.info/replication-http.html#implement-the-pullstream-endpoint).
