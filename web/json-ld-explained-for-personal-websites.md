...
created: 2026-06-22
description: "A guide to implementing JSON-LD on your personal website for better SEO, covering WebSite, Person, BlogPosting, and more with ready-to-use code snippets."
tags:
  - "clippings"
---

> [!summary]
> A practical guide to adding JSON-LD structured data to a personal website to improve SEO and help crawlers (including LLMs) understand your content. It walks through the core node types — `WebSite`, `WebPage`, `Person`, `ProfilePage`, `SoftwareApplication`, `BreadcrumbList`, `CollectionPage`, `Blog`, and `BlogPosting` — explaining how `@id`, `@type`, and linking properties tie nodes into a graph. Each section includes copy-paste-ready snippets, with `WebSite`, `ProfilePage`, and `Person` on the root page as the recommended minimum.
...
Here, a JSON object is initialised and the property `@context` is set to `https://schema.org`. In JSON-LD, the structure of data is determined by assigning the appropriate context. Web crawlers are standardised on [Schema.org,](https://schema.org/) which defines all the valid key-value pairs for the JSON.
...
![Google search results showing hawksley.dev, with the site's name "Ethan Hawksley" highlighted](https://hawksley.dev/_astro/ethan-hawksley-sitename.DLWamidF_15TJ9.avif)
...
There are more specific subtypes of `WebPage`. In this post, I’ll cover `ProfilePage` and `CollectionPage`. You can find less common ones at the bottom of [Schema.org’s definition for WebPage.](https://schema.org/WebPage)
...
- `sameAs` - Immensely useful for disambiguation, especially if you have a common name. It cleanly informs crawlers what your other profiles are, letting them build a knowledge graph representation of you across multiple pages. At the time of writing, [/g/11m62cgdtf](https://www.google.com/search?kgmid=/g/11m62cgdtf) is my Google knowledge graph ID.
...
There are lots of valid values for `applicationCategory`, you can find a list on [Google’s definition for SoftwareApplication.](https://developers.google.com/search/docs/appearance/structured-data/software-app)
...
![Google search results for The Shipwrecked Hackathon by Hack Club, with the breadcrumbs highlighted](https://hawksley.dev/_astro/hawksley-dev-breadcrumbs.B-inRf8E_1RmXaQ.avif)
...
![Definition of publisher property, where available types include Organization and Person](https://hawksley.dev/_astro/jsonld-publisher-organization-person.D6woCVfA_2nDeVz.avif)
