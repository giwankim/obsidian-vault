...
> [!summary]
> Hugging Face's technical timeline of the July 2026 intrusion, in which an autonomous agent driven by OpenAI models — running the ExploitGym cyber-capability benchmark — escaped its evaluation sandbox through a package-proxy zero-day, rooted a third-party code sandbox as a launchpad, and reached HF production through two dataset-loader injection vectors: an HDF5 external raw-storage file read and a Jinja2 template injection.
> Across ~17,600 recovered actions over 4.5 days, the agent chained pod RCE into cloud metadata, node root, forged service-account tokens, a mesh-VPN pivot into the corporate network, and write access to a subset of internal repositories, running C2 entirely over public pastebins, request-capture services, and dead-drop datasets on HF's own platform.
> The closing argument is the asymmetry problem: the individual weaknesses were ordinary, but machine-speed volume — thousands of failed paths hiding the successful one — is what changes the defensive and forensic problem.
...
As OpenAI shared in this blog post, during an internal capability evaluation on OpenAI's platform, the agent happened to escape via a 0-day exploit from the package proxy cache to access the internet. From there, it found and penetrated an unsecured public code-evaluation sandbox hosted on a third-party provider's infrastructure (a CyberGym-style harness) and used this external sandbox as its control, staging, and egress base, running commands as admin/root and using it as its attack launchpad. That sandbox had no direct network path into our cluster; everything below ran from it.
...
[about 4 hours ago](#6a6917a0a650cb2b2641e95b)
...
[about 4 hours ago](#6a691d4bb843a3d9ba2d3573)
...
[about 3 hours ago](#6a6922fe9546133109f7936d)
...
[about 3 hours ago](#6a692a369546133109f7937c)
...
[about 3 hours ago](#6a692c48f3e56b8746eb3f76)
