...
source: "https://tailscale.com/blog/how-nat-traversal-works?utm_source=substack&utm_medium=email"
author:
  - "[[David Anderson]]"
published: 2020-08-21
created: 2026-06-30
description: "Learn how NAT traversal works, how Tailscale can get through and securely connect your devices directly to each other."
tags:
  - "clippings"
---

> [!summary]
> A first-principles walkthrough (by Tailscale's David Anderson) of establishing direct peer-to-peer UDP connections across stateful firewalls and NATs. It builds up the full toolbox — simultaneous transmission to open firewalls, STUN for public endpoint discovery, the crucial distinction between "easy" (endpoint-independent) and "hard" (symmetric) NATs, birthday-paradox port guessing, UPnP/NAT-PMP/PCP port mapping, hairpinning and CGNAT, and IPv6/NAT64. Everything is tied together by ICE's elegant algorithm — "try every candidate path at once and pick the best one that works" — with TURN/DERP relays as the guaranteed fallback when direct connection fails.

![Branded artwork in greyscale](https://cdn.sanity.io/images/w77i7m8x/production/d68a410014ac19616bfa08f1faf458dd5e919dbf-600x315.svg?w=1200&q=75&fit=clip&auto=format)

We covered a lot of ground in our post about [*How Tailscale Works*](https://tailscale.com/blog/how-tailscale-works). However, we glossed over how we can get through NATs (Network Address Translators) and connect your devices directly to each other, no matter what’s standing between them. Let’s talk about that now!
...
NAT traversal with laptops diagram.
...
NAT traversal with app diagram.
...
NAT traversal diagram.

For UDP, the rule is very simple: the firewall allows an inbound UDP packet if it previously saw a matching outbound packet. For example, if our laptop firewall sees a UDP packet leaving the laptop from `2.2.2.2:1234` to `5.5.5.5:5678`, it’ll make a note that incoming packets from `5.5.5.5:5678` to `2.2.2.2:1234` are also fine. The trusted side of the world clearly intended to communicate with `5.5.5.5:5678`, so we should let them talk back.
...
NAT traversal diagram with firewall.
...
![Example of when NAT devices are more difficult, and create a completely different NAT mapping for every different destination.](https://cdn.sanity.io/images/w77i7m8x/production/c9edd473a0702412836a0f0efa1024b2df60a22e-2000x1076.png)
...
![Diagram of “carrier-grade NAT” (CGNAT).](https://cdn.sanity.io/images/w77i7m8x/production/96c154b056a1c0a992e131423c81fcfb1e6df368-2100x1080.png)

Diagram of “carrier-grade NAT” (CGNAT).
