...
> [!summary]
> A structural walkthrough of vLLM's V1 engine (analyzed at commit 42172ad, August 2025) that starts from a single-GPU offline `LLM.generate()` call and layers in complexity until it reaches async multi-node serving. It traces the engine constructor — config, processor, engine core client, output processor — into the scheduler's waiting/running queues and the KV-cache manager's `free_block_queue` that backs paged attention, then covers chunked prefill, prefix caching, guided decoding via FSM, speculative decoding, and disaggregated prefill/decode. The final third moves from `UniProcExecutor` to `MultiProcExecutor`, separates headless server nodes from API server nodes, and closes on latency-vs-throughput benchmarking and auto-tuning.
...
The best way to internalize this is to fire up your debugger and step through the codebase, but this section hopefully gives you a taste for it. This as well:![Drafting stage](https://www.aleksagordic.com/blog/vllm/specdec_pt1.png) ![Verify stage & rejection sampling stage](https://www.aleksagordic.com/blog/vllm/specdec_pt2.png)
