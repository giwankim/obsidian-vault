...
> [!summary]
> Replying to a physicist asking how to learn software design, matklad argues that design is learned by doing rather than from books, and that Conway's Law — software reproducing the social structure of the organization that builds it — matters more than the code itself. He illustrates with rust-analyzer, where build-system and feature-isolation decisions (stable-only builds, `catch_unwind`-guarded features, optimistic merging) were deliberately shaped to attract different kinds of contributors. He closes with recommended talks, essays, and books, cautioning that adapting to an incentive structure is usually more realistic than fixing it.
