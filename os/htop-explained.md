...
> [!summary]
> A deep, hands-on walkthrough of every field shown in htop/top on Linux, explaining where each value comes from by tracing it back to `/proc` files and system calls. Covers uptime, load averages, process IDs and the process tree, users and setuid, process states (R/S/D/Z/T/t), time slicing, niceness and priority, and the VIRT/RES/SHR/MEM memory columns. It closes with a survey of the default daemons running on a fresh Ubuntu server and whether each can safely be removed.
