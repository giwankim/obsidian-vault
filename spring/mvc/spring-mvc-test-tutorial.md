...
> [!summary]
> Petri Kainulainen's index page for his Spring MVC Test (MockMvc) tutorial series, covering unit and integration testing of Spring Boot controllers and REST APIs with JUnit 5. Links out to posts on standalone configuration, testing controllers that render views and forms, REST endpoints returning JSON, the newer MockMvcTester API, and integration-test setup with Maven and Spring profiles. Useful as a table of contents rather than a standalone article.
