...
source: "https://www.wimdeblauwe.com/blog/2026/07/07/testing-oauth2-client-login-with-mock-oauth2-server/?utm_source=RIECKPIL+Newsletter&utm_campaign=8a7921177c-january-monthly-update-2026_COPY_01&utm_medium=email&utm_term=0_d60be7e492-8a7921177c-446197869&mc_cid=8a7921177c"
author:
published: 2026-07-07
created: 2026-08-10
description: "If you use Spring Boot with Thymeleaf, you might be using an external OAuth2 provider like Keycloak or Okta to authenticate your users. In this blog post, I will show you how to test your OAuth2 client login with mock-oauth2-server."
tags:
  - "clippings"
---

> [!summary]
> How to test OAuth2 client login in a Spring Boot + Thymeleaf application using navikt's `mock-oauth2-server`, instead of booting Keycloak in a Testcontainer (slow to start and configure) or against a SaaS provider like Okta (which cannot be started from a test at all). It first walks through the real setup — a Keycloak `compose.yaml`, `spring.security.oauth2.client` provider/registration config with PKCE, and a `WebSecurityConfiguration` that maps client roles out of the ID token — then swaps in the mock server, started from an `ApplicationContextInitializer` that overrides the issuer URI for the test context. Full sources are in the author's `thymeleaf-mock-oauth2-login` example repository.

If you use Spring Boot with Thymeleaf, you might be using an external OAuth2 provider like Keycloak or Okta to authenticate your users. In this blog post, I will show you how to test your OAuth2 client login with [mock-oauth2-server](https://github.com/navikt/mock-oauth2-server).

If you use Keycloak, you could start a [Keycloak server in a Docker container](https://github.com/dasniko/testcontainers-keycloak) and use that for your tests. But that is not always the best option because it is slow to start, and you need to configure it. If you use a SaaS like Okta, you cannot start a server in your tests, so you need to mock the OAuth2 server.

With mock-oauth2-server, you can start a mock OAuth2 server in your tests and configure it to return the tokens you want.

## Application setup

Suppose you have a Spring Boot with Thymeleaf application created via [ttcli](https://github.com/wimdeblauwe/ttcli). To set up Keycloak locally, you can use the following `compose.yaml` file:
...
|  | If you want to use client roles, you need the following additional configuration in Keycloak:  1. Create the wanted client roles (e.g. `USER` and `ADMIN`). 2. Go to Clients > my-application-backend-client > Client scopes and click on `my-application-backend-client-dedicated`. Click on "Add predefined mapper" and select "client roles". Click "Add". 3. Click on "client roles" to edit the mapper. Set "Add to ID token" to "on". Save the changes. |
| --- | --- |
...
See [thymeleaf-mock-oauth2-login](https://github.com/wimdeblauwe/blog-example-code/tree/master/thymeleaf-mock-oauth2-login) on GitHub for the full sources of these examples.
