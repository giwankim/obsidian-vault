...
> [!summary]
> Explains the outbox pattern for reliably propagating data changes between microservices while avoiding the dual-write inconsistency between a database and Kafka. Within a single local transaction the service writes both its business data and an event row into an "outbox" table; Debezium then uses log-based change data capture to tail that table and publish events to Kafka, giving the source "read your own writes" semantics plus reliable, eventually-consistent, replayable cross-service messaging. Includes a full Java/Postgres/Kafka implementation covering topic routing via a custom SMT and consumer-side duplicate detection.
...
*Update (13 Sept. 2019):* To simplify usage of the outbox pattern, Debezium now provides a ready-to-use [SMT for routing outbox events](https://debezium.io/documentation/reference/0.9/configuration/outbox-event-router.html). The custom SMT discussed in this blog post is not needed any longer.

If you’ve built a couple of microservices, you’ll probably agree that the [hardest part about them is data](https://blog.christianposta.com/microservices/the-hardest-part-about-microservices-data/): microservices don’t exist in isolation and very often they need to propagate data and data changes amongst each other.
...
There are different approaches for letting the order service know the other two about new purchase orders; e.g. it could invoke some [REST](https://en.wikipedia.org/wiki/Representational_state_transfer), [grpc](https://grpc.io/) or other (synchronous) API provided by these services. This might create some undesired coupling, though: the sending service must know which other services to invoke and where to find them. It also must be prepared for these services temporarily not being available. Service meshes such as [Istio](https://istio.io/) can come in helpful here, by providing capabilities like request routing, retries, circuit breakers and much more.
...
[Log-based Change Data Capture](https://debezium.io/blog/2018/07/19/advantages-of-log-based-change-data-capture/) (CDC) is a great fit for capturing new entries in the outbox table and stream them to Apache Kafka. As opposed to any polling-based approach, event capture happens with a very low overhead in near-realtime. Debezium comes with [CDC connectors](https://debezium.io/docs/connectors/) for several databases such as MySQL, Postgres and SQL Server. The following example will use the [Debezium connector for Postgres](https://debezium.io/docs/connectors/postgresql).

You can find the complete [source code of the example](https://github.com/debezium/debezium-examples/tree/main/outbox) on GitHub. Refer to the [README.md](https://github.com/debezium/debezium-examples/blob/main/outbox/README.md) for details on building and running the example code. The example is centered around two microservices, [order-service](https://github.com/debezium/debezium-examples/tree/main/outbox/order-service) and [shipment-service](https://github.com/debezium/debezium-examples/tree/main/outbox/shipment-service). Both are implemented in Java, using [CDI](http://cdi-spec.org/) as the component model and JPA/Hibernate for accessing their respective databases. The order service runs on [WildFly](http://wildfly.org/) and exposes a simple REST API for placing purchase orders and canceling specific order lines. It uses a Postgres database as its local data store. The shipment service is based on [Thorntail](http://thorntail.io/); via Apache Kafka, it receives events exported by the order service and creates corresponding shipment entries in its own MySQL database. Debezium tails the transaction log ("write-ahead log", WAL) of the order service’s Postgres database in order to capture any new events in the outbox table and propagates them to Apache Kafka.

The overall architecture of the solution can be seen in the following picture:

![Outbox Pattern Overview](https://debezium.io/assets/images/outbox_pattern.png)

Note that the pattern is in no way tied to these specific implementation choices. It could equally well be realized using alternative technologies such as Spring Boot (e.g. leveraging Spring Data’s [support for domain events](https://docs.spring.io/spring-data/commons/docs/current/api/index.html?org/springframework/data/domain/DomainEvents.html)), plain JDBC or other programming languages than Java altogether.
...
Note how [Jackson’s](https://github.com/FasterXML/jackson) `ObjectMapper` is used to create a JSON representation of the event’s payload.
...
When using a tool such as the very practical [kafkacat](https://github.com/edenhill/kafkacat) utility, you should now see messages like these in the `OrderEvents` topic:
...
The `payload` field with the message values is the string-ified JSON representation of the original events. The Debezium Postgres connector emits `JSONB` columns as a string (using the `io.debezium.data.Json` logical type name), which is why the quotes are escaped. The [jq](https://stedolan.github.io/jq/) utility, and more specifically, its `fromjson` operator, come in handy for displaying the event payload in a more readable way:
...
![](https://debezium.io/assets/images/gmorling.jpg)
- [Next »](https://debezium.io/blog/2019/02/25/debezium-0-9-2-final-released/)

---

## About Debezium

Debezium is an open source distributed platform that turns your existing databases into event streams, so applications can see and respond almost instantly to each committed row-level change in the databases. Debezium is built on top of [Kafka](http://kafka.apache.org/) and provides [Kafka Connect](http://kafka.apache.org/documentation.html#connect) compatible connectors that monitor specific database management systems. Debezium records the history of data changes in Kafka logs, so your application can be stopped and restarted at any time and can easily consume all of the events it missed while it was not running, ensuring that all events are processed correctly and completely. Debezium is [open source](https://debezium.io/license/) under the [Apache License, Version 2.0](http://www.apache.org/licenses/LICENSE-2.0.html).

## Get involved

We hope you find Debezium interesting and useful, and want to give it a try. Follow us on Twitter [@debezium](https://twitter.com/debezium), [chat with us on Zulip](https://debezium.zulipchat.com/#narrow/stream/302529-users), or join our [mailing list](https://groups.google.com/forum/#!forum/debezium) to talk with the community. All of the code is open source [on GitHub](https://github.com/debezium/), so build the code locally and help us improve ours existing connectors and add even more connectors. If you find problems or have ideas how we can improve Debezium, please let us know or [log an issue](https://github.com/debezium/dbz/issues).
