...
created: 2026-06-21
description: "If you are building a microservice architecture, or you just need to send emails from your monolith, you should look into the Transactional Outbox pattern to ensure reliable communication between services.This blog post shows a few ways how you can implement this using Spring Boot."
tags:
  - "clippings"
---

> [!summary]
> The Transactional Outbox pattern keeps two systems in sync (e.g. saving an order and sending a confirmation email) without distributed transactions by persisting the intended external action in the same DB transaction, then dispatching it asynchronously with retries. The post shows two Spring Boot implementations: Spring Integration (JDBC-backed channel store, ordering preserved, safe across multiple instances) and Spring Modulith (event publication with readable JSON serialization, but manual/scheduled retries and no ordering guarantee). It compares the two on message ordering and multi-instance behavior, concluding Spring Integration has some advantages.
...
| **1** | Mark the method as an `@ApplicationModuleListener`. This is an annotation provided by Spring Modulith and a combination of:  - `@Async`: because we want the mail to be send asynchrounously. We don’t want the processing of the `CompleteOrder` use case to be affected by the email sending. - `@Transactional`: Since our listener runs in a separate thread, we should start a new transaction to get the state of the `Order` from the repository. - `@TransactionalEventListener`: This ensures this method is called when the transaction that contains the sending of the event is comitted. If the transaction is rolled back, our listener is not called. |
| --- | --- |
...
| id | listener\_id | event\_type | serialized\_event | publication\_date | completion\_date |
| --- | --- | --- | --- | --- | --- |
