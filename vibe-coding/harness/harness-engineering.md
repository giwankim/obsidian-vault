...
> [!summary]
> Explains harness engineering, Birgitta Boeckeler's idea of surrounding AI-assisted coding with deterministic tooling, agent-based review, and scheduled entropy checks so generated code keeps following a team's conventions. The three components are context engineering (a HARNESS.md that tells the AI what it must and must not do), architectural constraints enforced at verification slots, and periodic garbage collection, with each constraint maturing along a progressive-hardening ladder from unverified to agent-reviewed to deterministic. The AI Literacy Superpowers plugin implements this as three enforcement loops (advisory at edit time, strict at PR time, investigative on a schedule) plus a self-improving layer that mines session reflections and violation history to propose harness updates.
...
The ladder is one axis. **Reach** — whether a constraint is required on every PR or [complete-if-present](https://habitat-thinking.github.io/ai-literacy-superpowers/plugins/ai-literacy-superpowers/explanation/progressive-hardening/#rung-and-reach-are-different-axes) — is a second, and the `Enforcement` field does not record it.
...
For the document itself — what `HARNESS.md` is, how it is operated, and how it compares to `AGENTS.md` / CI / hooks — see [HARNESS.md, the Document](https://habitat-thinking.github.io/ai-literacy-superpowers/plugins/ai-literacy-superpowers/explanation/harness-md/).
