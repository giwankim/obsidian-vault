...
> [!summary]
> After repeating the same code-review corrections to LLMs session after session, Fabien Sanglard moved them into a project-root `agent.md` that the coding harness injects into every prompt. His ruleset covers terse human-facing prose, no magic numbers, early returns over nesting, short function names, enums instead of boolean parameters, strict layer boundaries with no hole-punching, minimal diffs, seven commit-message rules, and test-first bug fixing. He is clear it is not a silver bullet — context dilution (the "Lost in the Middle" effect) erodes adherence as sessions grow, mitigated by starting a new session per feature and asking the agent to reload `agent.md`.
...
Here is [my version of agent.md](https://fabiensanglard.net/agent.md/agent.md) as a starting point if you need one. Placing it in the root of a project should be enough. Alternatively, gemini.md/claude.md can be symlinked toward an *agent.md* to have it active anywhere.
