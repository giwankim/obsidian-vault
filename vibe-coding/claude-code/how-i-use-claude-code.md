...
> [!summary]
> Boris Tane's Claude Code workflow: research the codebase deeply, write a plan.md, annotate it iteratively (1-6 rounds) with inline corrections, then let Claude execute the full plan without stopping. The key principle is never letting Claude write code until a written plan is approved.
...
```
repeat 1-6xResearchPlanAnnotateTodo ListImplementFeedback & Iterate
```
...
```
NoYesClaude writes plan.mdI review in my editorI add inline notesSend Claude back to the documentClaude updates planSatisfied?Request todo list
```
...
```
NoYesYesNoClaude implementsI review / testCorrect?Terse correctionMore tasks?Done
```
...
```
Claude proposes changesI evaluate each itemAccept as-isModify approachSkip / removeOverride technical choiceRefined implementation scope
```
