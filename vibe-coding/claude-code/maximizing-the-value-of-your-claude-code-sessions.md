...
> [!summary]
> Breaks down what a Claude Code session actually costs: token price depends on the model, on input versus output (output runs roughly 5x input), and on prompt caching, where cache reads cost 0.1x but any change to the front of the request re-prefills everything. Switching `/model`, `/effort`, or fast mode mid-conversation busts the cache, so those decisions belong at the start of a session. The volume side comes down to what enters the context and how many turns it stays there, which argues for `/clear` between tasks, @-mentioning files instead of naming them, quiet flags on noisy commands, and handing high-output jobs to subagents.
