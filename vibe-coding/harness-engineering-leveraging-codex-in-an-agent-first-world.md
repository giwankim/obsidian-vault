...
> [!summary]
> OpenAI team built a product with 0 manually-written code using Codex agents, shipping ~1M lines across ~1,500 PRs in 5 months. Key lessons: treat AGENTS.md as a table of contents not an encyclopedia, enforce architecture mechanically via linters, make the app legible to agents via observability and Chrome DevTools, and use continuous "garbage collection" to fight entropy.
