...
created: 2026-08-04
description: "An interactive tour of what’s new in Go 1.27: every notable language, runtime, and standard library change, with short runnable examples you can edit and …"
tags:
  - "clippings"
---

> [!summary]
> A hands-on tour of Go 1.27 with runnable examples, continuing the interactive-tour series that Anton Zhiyanov retired after Go 1.26. The headline change is generic methods — a method can now declare its own type parameters independent of the receiver — alongside struct-literal field selectors for promoted fields, generalized function type inference, a standard library `uuid` package, post-quantum ML-DSA signatures, and `encoding/json/v2` graduating to back the classic `encoding/json` by default. It also covers size-specialized allocation, the graduated goroutine leak profile, experimental portable SIMD, and linker/tooling changes such as the removal of `h2_bundle.go` in favor of a real HTTP/2 package.

- [Go](https://victoriametrics.com/blog/categories/go/)
...
[`strings.Cut`](https://pkg.go.dev/strings@go1.27rc2#Cut) (from Go 1.18) splits around the *first* occurrence of a separator. Go 1.27 adds [`strings.CutLast`](https://pkg.go.dev/strings@go1.27rc2#CutLast) (and [`bytes.CutLast`](https://pkg.go.dev/bytes@go1.27rc2#CutLast)) for the *last* occurrence (a cleaner replacement for many [`LastIndex`](https://pkg.go.dev/strings@go1.27rc2#LastIndex) dances).
...
- [Go @ VictoriaMetrics](https://victoriametrics.com/blog/categories/go-@-victoriametrics/)
- [Open Source Tech](https://victoriametrics.com/blog/categories/open-source-tech/)
