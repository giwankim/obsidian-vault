...
source: "https://www.morling.dev/blog/revisiting-the-outbox-pattern/"
author:
published: 2024-10-31
created: 2026-06-27
description: "This post originally appeared on the Decodable blog. All rights reserved.Over the last few years, the outbox pattern has become a common solution for implementing data exchange flows between …"
tags:
  - "clippings"
---

> [!summary]
> Gunnar Morling's 2024 deep-dive revisiting the transactional outbox pattern for atomically updating a database and emitting Kafka events without distributed transactions. Argues log-based CDC (Debezium, or Postgres `pg_logical_emit_message()`) beats polling for ordering and overhead, rebuts criticisms around database load, complexity, and latency, and weighs alternatives (Dapr, read-yourself/Event Sourcing, raw CDC stream processing, Kafka 2PC via KIP-939, durable execution) — concluding the pattern remains a central tool.

*This post originally appeared on the [Decodable blog](https://www.decodable.co/blog/revisiting-the-outbox-pattern). All rights reserved.*
...
Let’s look at one of the business processes, the creation of appointments for a treatment, and how it could be implemented. Oh-my-Dawg runs a microservices architecture, with one service for managing appointments, and another one for managing inventory. When the appointment service receives a request via its REST API, it must persist the information about the requested treatment in its own database. We’re members of the [boring technology club](https://boringtechnology.club/), so Postgres is used as a data store. At the same time, the appointment service needs to notify the inventory service about the new appointment, so that shampoo and whatever else is needed can be reserved, and backordered, if needed. To decouple the two services, the notification is sent via [Apache Kafka](https://www.morling.dev/kafka).

![outbox pattern 1](https://www.morling.dev/images/outbox-pattern-1.webp)

Figure 1. The Oh-my-Dawg microservices architecture
...
![outbox pattern 2](https://www.morling.dev/images/outbox-pattern-2.webp)

Figure 2. The outbox pattern for atomically updating the Appointments table and emitting a notification
...
Note that, while the outbox pattern ensures atomicity of a local database update and sending out a message to external consumers, it does not provide complete [ACID transactional guarantees](https://en.wikipedia.org/wiki/ACID). Specifically, the pattern provides *eventual consistency* semantics: changes to the local datastore of the writing service become visible immediately, while the notification of other services happens asynchronously.
...
Writing an outbox message to the transaction log with `pg_logical_emit_message()`
...
These messages never materialize in any tables (and thus don’t cause any database growth apart from the WAL itself) and you also don’t need to take care of housekeeping, as any obsolete WAL segments will automatically be disposed of. If you are on Postgres, such a log-only outbox implementation is the one I’d recommend to use; you can learn more about the details in [this article](https://www.infoq.com/articles/wonders-of-postgres-logical-decoding-messages/). It shows how to implement the outbox pattern with Postgres logical decoding messages, using [Flink CDC](https://nightlies.apache.org/flink/flink-cdc-docs-stable/) and Debezium as a log-based outbox relay.
...
Persisting an appointment and emitting an outbox message in a Quarkus-based microservice
...
![outbox pattern 3](https://www.morling.dev/images/outbox-pattern-3.webp)

Figure 3. Joining "raw" change event streams (produced by Debezium and Flink CDC) via Flink SQL
