---
title: "자바 직렬화, 그것이 알고싶다. 실무편 | 우아한형제들 기술블로그"
source: "https://techblog.woowahan.com/2551/"
author:
published: 2017-10-17
created: 2026-08-12
description: "자바의 직렬화 기술에 대한 대한 두 번째 이야기입니다. 실제 자바 직렬화를 실무에 적용해보면서 주의해야 할 부분에 대해 이야기해보려고합니다. 자바 직렬화 실제 업무에서 사용해보기 자바 직렬화는 자바 개발자 입장에서는 상당히 쉽고 빠르게 사용할 수 있도록 만든 기술입니다. JSON 또는 CSV 등 형태의 포맷을 이용하면 직렬화 또는 역직렬화시에 특정 라이브러리를 도입해야 쉽게 개발이 가능하며, 구조가 복잡하면 직접"
tags:
  - "clippings"
---

> [!summary]
> The practical follow-up in Woowa Brothers' Java serialization series, cataloging what goes wrong when `Serializable` objects outlive the classes that wrote them. An auto-generated `serialVersionUID` is a hash of the class structure, so adding one field throws `InvalidClassException`; even with a pinned SUID, field *type* changes (`String` → `StringBuilder`, `int` → `long`) fail hard, while additions and removals pass silently but drop data. It also measures the size penalty — 146 bytes vs 62 for JSON on a trivial object, since class metadata travels with the payload — and argues against Java serialization for anything long-lived in Redis/Memcached or for framework classes like Spring Security's `SecurityContextImpl`, whose SUID tracks the library version.

## 자바 직렬화, 그것이 알고싶다. 실무편

2017\. 10. 17. WoowaTech ![](https://www.gravatar.com/avatar/34f5077a5a7489fb3d604ee7d75699d9)

[Backend](https://techblog.woowahan.com/?pcat=backend)

> 자바의 직렬화 기술에 대한 대한 두 번째 이야기입니다.
...
우리가 보통 원하는 것은 `phone` 멤버 변수가 추가되어도 기존 멤버 변수의 기존 멤버 변수는 채워지길 원합니다. `phone` 은 `null` 되어 있더라도 말이죠.
...
그래서 자바 직렬화 스펙을 확인해보았습니다. ([링크](http://docs.oracle.com/javase/6/docs/platform/serialization/spec/class.html#4100))
...
([링크](http://docs.oracle.com/javase/6/docs/platform/serialization/spec/class.html#4100) 를 보면 알 수 있지만 해쉬값은 클래스 구조 정보를 이용해서 생성하는 것을 알 수 있습니다.)
...
자바 직렬화를 사용할 때 클래스 구조 변경 시 어떤 부분을 확인해야 할지 **정리** 해보겠습니다.
...
- 개발자가 직접 컨트롤이 가능한 클래스의 객체가 아닌 클래스의 객체에 대해서는 직렬화를 지양해야 합니다.
	...
	위와 관련된 예시는 [스프링 시큐리티](https://github.com/spring-projects/spring-security) 의 `SecurityContextImpl` 클래스가 있습니다.
	`SecurityContext` 를 구현한 클래스 클래스로 [링크](https://github.com/spring-projects/spring-security/tree/master/core/src/main/java/org/springframework/security/core/context/SecurityContextImpl.java) 를 보면 확인할 수 있습니다.
...
용량 문제는 생각보다 많은 곳에서 나타나는 문제입니다. 특히 직렬화된 데이터를 메모리 서버([Redis](https://redis.io/), Memcached)에 저장하는 형태를 가진 시스템에서 두드러집니다.
...
- Java Object Serialization Specification version 6.0
	- [https://docs.oracle.com/javase/8/docs/platform/serialization/spec/serial-arch.html](https://docs.oracle.com/javase/8/docs/platform/serialization/spec/serial-arch.html)
