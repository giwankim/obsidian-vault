...
source: "https://s4dbrd.github.io/posts/how-kernel-anti-cheats-work/"
author:
  - "[[s4dbrd]]"
...
> [!summary]
> A deep technical exploration of modern kernel-level anti-cheat systems used in gaming, covering how they operate at Ring 0 privilege, intercept kernel callbacks, scan memory structures, and detect cheats. Discusses major systems like BattlEye, EasyAntiCheat, and Vanguard, as well as advanced evasion techniques including PCIe DMA attacks.
...
**Shared memory sections** created with [`NtCreateSection`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwcreatesection) and mapped into both the service process and the game process via [`NtMapViewOfSection`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwmapviewofsection) allow high-bandwidth, low-latency data sharing. Telemetry data (input events, timing data) can be written to a shared ring buffer by the game DLL and read by the service without the overhead of IPC per event.

The distinction between boot-time and runtime driver loading is more significant than it might appear.

**BattlEye and EAC** load their kernel drivers when the game is launched. `BEDaisy.sys` and its EAC equivalent are registered as demand-start drivers and loaded via [`ZwLoadDriver`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwloaddriver) from the service when the game starts. They are unloaded when the game exits.
...
[![BattlEye Architecture](https://s4dbrd.github.io/assets/img/posts/battleye-architecture.png)](https://s4dbrd.github.io/assets/img/posts/battleye-architecture.png) *BattlEye’s three-component architecture: BEDaisy.sys at ring 0 communicates upward via IOCTLs to BEService.exe running as a SYSTEM service, which manages BEClient\_x64.dll injected in the game process.*
...
[`ObRegisterCallbacks`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-obregistercallbacks) is perhaps the single most important API for process protection. It allows a driver to register a callback that is invoked whenever a handle to a specified object type is opened or duplicated. For anti-cheat purposes, the object types of interest are `PsProcessType` and `PsThreadType`.
...
The pre-operation callback receives a [`POB_PRE_OPERATION_INFORMATION`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/ns-wdm-_ob_pre_operation_information) structure. The critical field is `Parameters->CreateHandleInformation.DesiredAccess`. The callback can strip access rights from the desired access by modifying `Parameters->CreateHandleInformation.DesiredAccess` before the handle is created. This is how anti-cheats prevent external processes from opening handles to the game process with `PROCESS_VM_READ` or `PROCESS_VM_WRITE` access.

When a cheat calls [`OpenProcess`](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-openprocess) `(PROCESS_VM_READ | PROCESS_VM_WRITE, FALSE, gameProcessId)`, the anti-cheat’s `ObRegisterCallbacks` pre-operation callback fires. The callback checks whether the target process is the protected game process. If it is, it strips `PROCESS_VM_READ`, `PROCESS_VM_WRITE`, `PROCESS_VM_OPERATION`, and `PROCESS_DUP_HANDLE` from the desired access. The cheat receives a handle, but the handle is useless for reading or writing game memory. The cheat’s [`ReadProcessMemory`](https://learn.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-readprocessmemory) call will fail with `ERROR_ACCESS_DENIED`.
...
[![ObRegisterCallbacks demo](https://s4dbrd.github.io/assets/img/posts/obcallback-demo.png)](https://s4dbrd.github.io/assets/img/posts/obcallback-demo.png) *ObCallbackDemo.sys in action. DebugView shows the driver stripping handle access rights, Target.exe is running with secret data in memory, and Verifier.exe fails to read it with Access Denied.*

### PsSetCreateProcessNotifyRoutineEx

[`PsSetCreateProcessNotifyRoutineEx`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-pssetcreateprocessnotifyroutineex) allows a driver to register a callback that fires on every process creation and termination event system-wide. The callback receives a `PEPROCESS` for the process, the PID, and a [`PPS_CREATE_NOTIFY_INFO`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/ns-ntddk-_ps_create_notify_info) structure containing details about the process being created (image name, command line, parent PID).
...
The call to [`PsLookupThreadByThreadId`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntifs/nf-ntifs-pslookupthreadbythreadid) retrieves the `ETHREAD` pointer for the new thread. `PsGetThreadWin32StartAddress` returns the Win32 start address as seen by the process, which is distinct from the kernel-internal start address. Once finished with the thread object, [`ObDereferenceObject`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-obdereferenceobject) releases the reference acquired by `PsLookupThreadByThreadId`.
...
[`PsSetLoadImageNotifyRoutine`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-pssetloadimagenotifyroutine) fires whenever an image (DLL or EXE) is mapped into any process. It provides the image file name and a [`PIMAGE_INFO`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/ns-ntddk-_image_info) structure containing the base address and size.
...
[`CmRegisterCallbackEx`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-cmregistercallbackex) registers a callback for registry operations. Anti-cheats use this to monitor for registry modifications that might indicate cheats configuring themselves or attempting to modify anti-cheat settings.
...
The [`KeStackAttachProcess`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntifs/nf-ntifs-kestackattachprocess) / [`KeUnstackDetachProcess`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntifs/nf-ntifs-keunstackdetachprocess) pattern is used to temporarily attach the calling thread to the target process’s address space, allowing the driver to read memory that is mapped into the game process without going through handle-based access controls. [`RtlImageNtHeader`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntimage/nf-ntimage-rtlimagentheader) parses the PE headers from the in-memory image base.
...
Anti-cheats walk the VAD tree directly rather than relying on [`ZwQueryVirtualMemory`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntifs/nf-ntifs-zwqueryvirtualmemory), because the VAD tree cannot be trivially hidden from kernel mode in the same way that module lists can be manipulated. Walking the VAD:
...
[![VAD tree showing injected RWX region](https://s4dbrd.github.io/assets/img/posts/vad-injected-rwx-region.png)](https://s4dbrd.github.io/assets/img/posts/vad-injected-rwx-region.png) *The first entry is a Private EXECUTE\_READWRITE region with no backing file, injected by our test tool. Every legitimate module shows as Mapped Exe with a full file path.*
...
The classic injection technique: call [`CreateRemoteThread`](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createremotethread) in the target process with `LoadLibraryA` as the thread start address and the DLL path as the argument. This is trivially detectable via `PsSetCreateThreadNotifyRoutine`: the new thread’s start address will be `LoadLibraryA` (or rather its address in kernel32.dll), and the caller process is not the game itself.
...
[`QueueUserAPC`](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-queueuserapc) and the underlying `NtQueueApcThread` allow queuing an Asynchronous Procedure Call to a thread in any process for which the caller has `THREAD_SET_CONTEXT` access. When the target thread enters an alertable wait, the APC fires and executes arbitrary code in the target thread’s context.

Detection at the kernel level leverages the [`KAPC`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/ns-wdm-_kapc) structure. Each thread has a kernel APC queue and a user APC queue. Anti-cheats can inspect the pending APC queue of game process threads to detect suspicious APC targets:
...
A sophisticated injection technique maps a shared section object (backed by a file or created with [`NtCreateSection`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwcreatesection)) into the target process using [`NtMapViewOfSection`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwmapviewofsection). This bypasses `CreateRemoteThread` -based detection heuristics because no remote thread is created initially. The injected code is then typically triggered via APC or by modifying an existing thread’s context.
...
[![Target.exe and inject_pe setup](https://s4dbrd.github.io/assets/img/posts/reflective-dll-setup.png)](https://s4dbrd.github.io/assets/img/posts/reflective-dll-setup.png)
...
[![VAD tree and legitimate PE header](https://s4dbrd.github.io/assets/img/posts/reflective-dll-vad-legit.png)](https://s4dbrd.github.io/assets/img/posts/reflective-dll-vad-legit.png)
...
[![Injected PE header](https://s4dbrd.github.io/assets/img/posts/reflective-dll-injected-pe.png)](https://s4dbrd.github.io/assets/img/posts/reflective-dll-injected-pe.png)
...
[![PEB module list](https://s4dbrd.github.io/assets/img/posts/reflective-dll-peb-modules.png)](https://s4dbrd.github.io/assets/img/posts/reflective-dll-peb-modules.png)

### Stack Walking with RtlWalkFrameChain

When BEDaisy wants to inspect a thread’s call stack, it uses an APC mechanism to capture the stack frames while the thread is in user mode. The APC fires in the game thread’s context and calls [`RtlWalkFrameChain`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-rtlwalkframechain) or [`RtlCaptureStackBackTrace`](https://learn.microsoft.com/en-us/windows/win32/api/winnt/nf-winnt-rtlcapturestackbacktrace) to capture the return address chain.
...
[![Target.exe and inject_hook setup](https://s4dbrd.github.io/assets/img/posts/inline-hook-setup.png)](https://s4dbrd.github.io/assets/img/posts/inline-hook-setup.png)
...
[![Clean NtReadVirtualMemory prologue](https://s4dbrd.github.io/assets/img/posts/inline-hook-before.png)](https://s4dbrd.github.io/assets/img/posts/inline-hook-before.png)
...
[![Hooked NtReadVirtualMemory](https://s4dbrd.github.io/assets/img/posts/inline-hook-after.png)](https://s4dbrd.github.io/assets/img/posts/inline-hook-after.png)
...
The `SeValidateImageHeader` and `SeValidateImageData` functions in the kernel validate driver signatures. Anti-cheats can inspect loaded driver objects and verify that their [`IMAGE_INFO_EX`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/ns-ntddk-_image_info_ex) `ImageSignatureType` and `ImageSignatureLevel` fields reflect proper signing.
...
1. The attacker finds a legitimate, signed driver with a vulnerability (typically a dangerous IOCTL handler that allows arbitrary kernel memory reads/writes, or that calls [`MmMapIoSpace`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-mmmapiospace) with attacker-controlled parameters).
...
[![PiCompareDDBCacheEntries in Binary Ninja](https://s4dbrd.github.io/assets/img/posts/piddb-binja-compare.png)](https://s4dbrd.github.io/assets/img/posts/piddb-binja-compare.png)
...
[![PiDDBCacheTable AVL table metadata](https://s4dbrd.github.io/assets/img/posts/piddb-avl-table.png)](https://s4dbrd.github.io/assets/img/posts/piddb-avl-table.png)
...
[![Walking the balanced links](https://s4dbrd.github.io/assets/img/posts/piddb-balanced-links.png)](https://s4dbrd.github.io/assets/img/posts/piddb-balanced-links.png)
...
[![Cached driver entries](https://s4dbrd.github.io/assets/img/posts/piddb-cache-entries.png)](https://s4dbrd.github.io/assets/img/posts/piddb-cache-entries.png)
...
At the usermode level (in the game-injected DLL), the anti-cheat uses [`NtQueryInformationProcess`](https://learn.microsoft.com/en-us/windows/win32/api/winternl/nf-winternl-ntqueryinformationprocess) with multiple information classes:
...
Some anti-cheats go further and directly inspect the `KDDEBUGGER_DATA64` structure and the shared kernel data page ([`KUSER_SHARED_DATA`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/ns-ntddk-kuser_shared_data)) for debugger-related flags.
...
[![Creating a hidden thread](https://s4dbrd.github.io/assets/img/posts/hidethread-setup.png)](https://s4dbrd.github.io/assets/img/posts/hidethread-setup.png)
...
[![CrossThreadFlags before](https://s4dbrd.github.io/assets/img/posts/hidethread-before.png)](https://s4dbrd.github.io/assets/img/posts/hidethread-before.png)
...
[![CrossThreadFlags after](https://s4dbrd.github.io/assets/img/posts/hidethread-after.png)](https://s4dbrd.github.io/assets/img/posts/hidethread-after.png)
...
Anti-cheats scan all threads’ saved debug register state (accessible via [`CONTEXT`](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-context) structure obtained with [`KeGetContextThread`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-kegetcontextthread) or directly from `KTHREAD::TrapFrame`) for active hardware breakpoints not set by the anti-cheat itself.
...
[![DMA cheat architecture](https://s4dbrd.github.io/assets/img/posts/dma-cheat-architecture.png)](https://s4dbrd.github.io/assets/img/posts/dma-cheat-architecture.png) *The FPGA sits on the game PC’s PCIe bus and reads physical memory via TLPs without CPU involvement. The cheat software runs entirely on a separate machine, leaving the game PC clean from a software perspective.*
...
[![Mouse trajectory comparison](https://s4dbrd.github.io/assets/img/posts/mouse-trajectory-comparison.png)](https://s4dbrd.github.io/assets/img/posts/mouse-trajectory-comparison.png) *Left: a legitimate player’s mouse path follows Fitts’ Law with a natural S-curve, overshoot, and micro-corrections. Right: an aimbot produces an idle phase followed by an instant linear snap to the target with no natural deceleration.*
...
- **SMBIOS data**: Manufacturer, product name, serial numbers, UUID. Accessed via [`NtQuerySystemInformation`](https://learn.microsoft.com/en-us/windows/win32/api/winternl/nf-winternl-ntquerysysteminformation) `(SystemFirmwareTableInformation, ...)` or directly via the firmware table.
- **Disk serial numbers**: Physical disk serial numbers via IOCTL [`IOCTL_STORAGE_QUERY_PROPERTY`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddstor/ni-ntddstor-ioctl_storage_query_property). These are stable identifiers that survive OS reinstallation.
...
- **Kernel callbacks** ([`ObRegisterCallbacks`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-obregistercallbacks), [`PsSetCreateProcessNotifyRoutineEx`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-pssetcreateprocessnotifyroutineex), [`PsSetLoadImageNotifyRoutine`](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/nf-ntddk-pssetloadimagenotifyroutine)) provide real-time visibility into system events with the ability to actively block malicious operations.
...
1. Collins, R. et al. “Anti-Cheat: Attacks and the Effectiveness of Client-Side Defences.” *CheckMATE 2024 (Workshop co-located with CCS 2024)*. [https://tomchothia.gitlab.io/Papers/AntiCheat2024.pdf](https://tomchothia.gitlab.io/Papers/AntiCheat2024.pdf)
2. Vella, R. et al. “If It Looks Like a Rootkit and Deceives Like a Rootkit: A Critical Analysis of Kernel-Level Anti-Cheat Systems.” *ARES 2024*. [https://arxiv.org/pdf/2408.00500](https://arxiv.org/pdf/2408.00500)
3. Sousa, J. et al. “AntiCheatPT: A Transformer-Based Approach to Cheat Detection in First-Person Shooter Games.” *2025*. [https://arxiv.org/html/2508.06348v1](https://arxiv.org/html/2508.06348v1)
4. secret.club. “Reversing BattlEye’s anti-cheat kernel driver.” *2019*. [https://secret.club/2019/02/10/battleye-anticheat.html](https://secret.club/2019/02/10/battleye-anticheat.html)
5. secret.club. “Easy Anti-Cheat integrity check bypass.” *2020*. [https://secret.club/2020/04/08/eac\_integrity\_check\_bypass.html](https://secret.club/2020/04/08/eac_integrity_check_bypass.html)
6. back.engineering. “Reversing BEDaisy.sys.” *2020*. [https://back.engineering/blog/2020/08/22/](https://back.engineering/blog/2020/08/22/)
7. Aki2k. “BEDaisy Reverse Engineering.” *GitHub*. [https://github.com/Aki2k/BEDaisy](https://github.com/Aki2k/BEDaisy)
8. archie-osu. “Vanguard Dispatch Table Hooks Analysis.” *2025*. [https://archie-osu.github.io/2025/04/11/vanguard-research.html](https://archie-osu.github.io/2025/04/11/vanguard-research.html)
9. rhaym-tech. “Vanguard vgk.sys Analysis Gist.” *GitHub Gist*. [https://gist.github.com/rhaym-tech/f636b76deeca15528e70304b5ee95980](https://gist.github.com/rhaym-tech/f636b76deeca15528e70304b5ee95980)
10. donnaskiez. “ac: Open Source Kernel Anti-Cheat.” *GitHub*. [https://github.com/donnaskiez/ac](https://github.com/donnaskiez/ac)
