...
> [!summary]
> Mitchell Hashimoto argues that everyday SIMD is far simpler than its reputation suggests, and that any hot loop scanning or transforming large contiguous data follows the same five-step shape: broadcast constants, loop one vector width at a time, do the operation across all lanes, reduce the vector result, then handle the remainder with a scalar tail. He demonstrates with a real Ghostty example in Zig — finding the first codepoint at or below `0xF` — where 12 extra lines using `@splat`, `@reduce`, `@bitCast`, and `@ctz` yield up to 16x throughput depending on the target ISA, and about 5x end-to-end on AVX2. He also explains why you shouldn't rely on compiler auto-vectorization: production compilers regularly miss opportunities, and explicit vectorization won't silently regress on an unrelated code change.
