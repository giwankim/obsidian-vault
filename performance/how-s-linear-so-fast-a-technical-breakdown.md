...
> [!summary]
> A reverse-engineered breakdown of why Linear feels instant: it treats the browser's IndexedDB as the primary database, applying mutations locally first via MobX observables and syncing to the server in the background, so the UI never waits on the network. First-load speed comes from aggressive code splitting, module preloading, a service-worker precache, and an inlined app shell that renders before authentication is even verified. Rounding it out are keyboard-first design with a command palette over the in-memory store, and disciplined GPU-only animations with sub-100ms durations.
...
Another cornerstrone to Linear's speed is how they've intergarated the keyboard as a primary tool to navigate and complete your work. Every common action has a shortcut. The command palette is one keystroke away. The right-click menu is custom-built. None of these are accidents but instead thoughtful design decision from day one.
...
There are no transitions on list items to keep things snappy
