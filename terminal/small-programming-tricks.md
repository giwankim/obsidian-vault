...
> [!summary]
> Will Keleher argues that much of engineering productivity comes from small, high-leverage nuggets of knowledge that need little background to use. Examples include fuzzy shell-history search (fzf, atuin), `SELECT` without `FROM`, `EXPLAIN ANALYZE`, regex `\b`, log-bucketed metrics, `git log -S` (pickaxe), `git checkout -`, globstar instead of `find`, and ripgrep. He suggests senior engineers share one such trick a day, including company-specific ones like which data source to use for debugging a given problem.
...
[^1]: I think I first saw this technique on [Julia Evan’s blog](https://jvns.ca/), and I think her writing often perfectly encapsulates the idea behind this post: "small bits of knowledge are powerful! and fun! and approachable!!"
