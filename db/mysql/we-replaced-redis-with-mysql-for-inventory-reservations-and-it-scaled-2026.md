---
title: "We replaced Redis with MySQL for inventory reservations—and it scaled (2026)"
source: "https://shopify.engineering/scaling-inventory-reservations"
author:
  - "[[Emilie Noel]]"
published: 2001-05-12
created: 2026-08-09
description: "How we used SKIP LOCKED, composite primary keys, and connection visibility to hit our scale targets."
tags:
  - "clippings"
---

> [!summary]
> Shopify moved checkout oversell protection from Redis to MySQL so reservations and the inventory ledger could share ACID transactions, using `SELECT ... FOR UPDATE SKIP LOCKED` over one row per sellable unit with a bounded pool capped at 1,000 rows per item/location. Getting it to scale needed a composite primary key (one row lock instead of two), `READ COMMITTED` to dodge supremum gap locks during replenishment, consistent lock ordering across reserve and claim to kill deadlocks, and `UNION ALL` batching to cut round trips. The real ceiling turned out to be connection hold time in unrelated checkout code, found by tagging SQL with `/* conn_tag:... */` and aggregating per-caller hold time at the ProxySQL layer.
...
Earlier attempts had failed. A single row with a quantity column couldn't handle the contention. MySQL 8's `SKIP LOCKED` feature introduced a different design: one row per inventory unit instead of one row per item. Inspired by [37signals' approach](https://dev.37signals.com/introducing-solid-queue/ "37signals: Introducing Solid Queue") to database-backed load distribution, we rebuilt reservations on MySQL and hit our high-throughput targets during peak 2025 traffic.
...
We changed the transaction isolation level from `REPEATABLE READ` (MySQL default) to `READ COMMITTED` for these transactions. Under `READ COMMITTED`, InnoDB doesn't take gap locks in the same way, so replenishment could proceed. Jahfer Husain's [guide to InnoDB locking](https://jahfer.com/posts/innodb-locks/ "Jahfer Husain: InnoDB locking") was very helpful for understanding this. This was our first use of a non-default isolation level in this codebase; it required small framework support for setting isolation per transaction.
...
The fix was to standardize the order: reserve always `DELETE` s from the units table first, then `INSERT` s into `reserved_quantities`. Claim only touches `reserved_quantities`. Because both paths now acquire locks in the same order, neither can hold a lock the other is waiting for—no more circular waits.
...
![Water line vs race meme: inventory](https://cdn.shopify.com/s/files/1/0779/4361/files/water_line_vs_race.png?v=1778597595)
