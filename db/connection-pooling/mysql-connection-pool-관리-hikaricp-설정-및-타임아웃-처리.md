...
source: "https://cheese10yun.github.io/mysql-connection-pool-timeout/"
author:
  - "[[Yun]]"
published: 2024-10-21
created: 2026-07-19
description: "애플리케이션에서 MySQL 데이터베이스와의 효율적인 연결을 위해 커넥션 풀을 사용합니다. 커넥션 풀은 미리 일정한 수의 데이터베이스 연결을 생성해 두고, 애플리케이션이 필요할 때마다 이 연결들을 재사용함으로써 성능을 향상시키는 기법입니다."
tags:
  - "clippings"
---

> [!summary]
> Korean guide to managing MySQL connection pools with HikariCP in Spring Boot, including a reference table of every Hikari setting and its default (maximum-pool-size 10, connection-timeout 30s, max-lifetime 30min, idle-timeout 10min, etc.). Walks through pool-state fields — activeConnections, idleConnections, totalConnections, threadsAwaitingConnection — with a scenario showing what happens as concurrent requests exhaust the pool. Includes Kotlin sample code for reading live pool state via HikariPoolMXBean/HikariConfigMXBean.
